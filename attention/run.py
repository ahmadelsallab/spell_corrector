...
def main(args):
    ...
    '''
    vocab = Vocabulary(text_file=args.training_data, padding=args.padding)
    print('Loading datasets.')
    #print(vocab.vocabulary)
    input_vocab = vocab
    output_vocab = vocab
    training = Data(args.training_data, input_vocab, output_vocab, args.delimiter)
    validation = Data(args.validation_data, input_vocab, output_vocab, args.delimiter)
    training.load()
    validation.load()
    training.transform()
    ...
    n_samples = 100
    cnt = 0
    for ex in validation.inputs:
        if cnt < n_samples:
            cnt += 1
            text = ''.join(vocab.int_to_string(ex))
            test_examples.append(text)
    run_examples(model, input_vocab, output_vocab, examples=test_examples)
...
if __name__ == '__main__':
    ...
    named_args.add_argument('-d', '--delimiter', metavar='|',
                            help="""Separator of input|target""",
